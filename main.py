...
subparsers = parser.add_subparsers(description="Specifies an action to do", required=True, dest='action')

parent_io = argparse.ArgumentParser(add_help=False)
parent_io.add_argument('input', help="Path to input file")
parent_io.add_argument('output', nargs='?', help='Path to save the output file. Defaults to saving next to the input')

parser_compress = subparsers.add_parser('compress', help="Compresses an image", aliases=['c'], parents=[parent_io, compress_args])
parser_extract = subparsers.add_parser('extract', help="Extracts image from an archive", aliases=['e'], parents=[parent_io])
...
parser_test.add_argument('input', help='Path to input file')
parser_test.add_argument('archive_output', nargs='?', help='Path to save the output archive. Defaults to saving next to the input')
parser_test.add_argument('image_output', nargs='?', help='Path to save the output image. Defaults to saving next to the output archive')
...
if args.action == 'compress':
  compress(args.input, args.output or replace_ext(args.input, '.zip'))
elif args.action == 'extract':
  extract(args.input, args.output or replace_ext(args.input, '.png'))
elif args.action == 'test':
  args.archive_output = args.archive_output or replace_ext(args.input, '.zip')
  compress(args.input, args.archive_output)
  extract(args.archive_output, args.image_output or replace_ext(args.archive_output, '.png'))
